function [X,Y,Out] = lmafit_mc_adp_single(m,n,k,Known,data,opts)
%
...
DoQR = true;
est_rank = 1;
rank_max =  min(m,n)-1;%max(floor(0.1*min(m,n)),2*k);
rank_min =  1;
...
if Zfull %Z is full
    ...
    Z = zeros(m,n,'single');  Z(Known) = data;   
...
if init == 0
    X = zeros(m,k,'single');   Y = eye(k,n,'single');   Res = data;   res = datanrm; 
...
for iter = 1:maxit
    ...
    elseif ratio > 0.7; %ratio > 0.7;
        increment = max(increment, 0.25*alf);   % increment = max(increment, 0.25*alf);   
        alf = alf + increment;
    end

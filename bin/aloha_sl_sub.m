function [rimg_n,rimg_E,itermap]=...
    aloha_sl_sub(dimg,mask,mid,Nimg,Nfir,Nc,mu,beta,tau,...
...
for iter=1:N
    ...
    switch opt_inc
        ...
        case 'dec'
            % decreading strategy
            opts.est_rank=1;
            [U,V,~] = lmafit_mc_adp_single(m,n,n-1,ids,data_ids,opts);
    end
    r=size(U,2);
    ...
    U   = gpuArray(single(U));
    V   = gpuArray(single(V'));
    [X,E,endi]=admm_hankel_sl(U,V,meas,meas_id,mu,beta,tau,muiter,Nimg,Nimg,Nc,tolE_stop,H_gpu,Hi_gpu);
